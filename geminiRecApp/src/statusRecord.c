 ...
 * History
 * -------
 * Version 1.0  11-11-91  jba   Created. Moved set and reset of alarm stat and sevr to macros.
 * Version 1.1  02-05-92  jba	Changed function arguments from paddr to precord.
 * Version 1.2  02-28-92  jba   Changed get_precision,get_graphic_double,get_control_double.
 * Version 1.3  02-28-92  jba	ANSI C changes.
 * Version 1.4  04-10-92  jba   Pact now used to test for asyn processing, not status.
 * Version 1.5  04-18-92  jba   Removed process from dev init_record parms.
 * Version 1.6  06-02-92  jba   Changed graphic/control limits for hihi,high,low,lolo.
 * Version 1.7  07-15-92  jba   Changed VALID_ALARM to INVALID alarm.
 * Version 1.8  07-16-92  jba   Added invalid alarm fwd link test and chngd fwd lnk to macro.
 * Version 1.9  07-21-92  jba   Changed alarm limits for non val related fields.
 * Version 2.0  08-06-92  jba   New algorithm for calculating analog alarms.
 * Version 2.1  08-13-92  jba   Added simulation processing.
 * Version 2.2  05-30-92  vong	Convert recLongin to recStatus.
 * Version 2.3	06-18-95  tac	Fixed bug in bit16 & bit24 - bit31.
 * Version 2.4  02-26-96  tac	Process links on change of stat and sevr.
 *				Decode bits before processing links. Fixed bug in bit 24.
 * Version 2.5  03-16-01  ajf   Changes for 3.13.
 *                              Added get_precision function for VERS field.
 * Version 2.6  11/12/03  ajf   Changes to comply with the new macro for "dbGetLink"
 *                              in EPICS 3.13.9.
...
static long init_record( statusRecord *pstatus, int pass )
{
...
static long process( statusRecord *pstatus )
{
...
static void checkAlarms( statusRecord *pstatus )
{
	double		val;
...
static void monitor( statusRecord *pstatus )
{
...
static long readValue( statusRecord *pstatus )
{
	long		status;
        struct statusdset 	*pdset = (struct statusdset *) (pstatus->dset);
	long            nRequest;
	...
	nRequest = 1;
	options  = 0;
        status   = dbGetLink( &(pstatus->siml), DBR_ENUM, &(pstatus->simm), &options, &nRequest );
	if (status)
		return(status);
	...
	if (pstatus->simm == menuYesNoYES){
	        nRequest = 1;
	        options  = 0;
                status   = dbGetLink( &(pstatus->siol), DBR_LONG, &(pstatus->sval), &options, &nRequest );
		if (status==0){
...
static void decodeBits( statusRecord *pstatus )
{
...
static void activateNewLink( statusRecord *pstatus )
{
  if (pstatus->lval != pstatus->val) {
    if ((pstatus->lval & 0x0000ffff) != (pstatus->val & 0x0000ffff)) {
...
static void activateAllLink( statusRecord *pstatus )
{
	  if (pstatus->lk00.type == DB_LINK) {
            dbScanFwdLink( &(pstatus->lk00) );
	  }
